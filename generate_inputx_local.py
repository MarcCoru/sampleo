...
parser.add_argument('-i','--indir', type=str, required=True, help='Input tfrecord file from GEE')
...
def read_tfrecord_srdata(filename, indir):
    
    # reads a tfRecord file from Google Storage
    tf_file = indir + '/' + filename
    print(tf_file)
...
def read_tfrecord_envdata(filename, indir, doy):
...
# create outdirs
outdir = indir + '/' + dataset + '/gz/' + str(int(ksize/maxblocks)) + '/' 
if envdata == True:
    outdir += "bio" + '/'
else:
    outdir += "nonbio" + '/'
...
indir_srdata = os.path.join(indir, dataset,'raw',str(ksize),"sr_data","data"+ tyear[2:])
b1, b2, b3, b4, b5, b6, b7, b8, b8a, b9, b10, b11, b12, year, doy = read_tfrecord_srdata(infile, indir_srdata)
...
if envdata == False:
    ...
    bio01, bio12, srtm  = read_tfrecord_envdata(infile, indir_aux, doy)
